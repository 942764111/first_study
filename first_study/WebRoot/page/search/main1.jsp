...
<%
	...
	String searchType = String.valueOf(request.getAttribute("searchType"));
	System.out.println("searchType:"+searchType);
	String TCName = String.valueOf(request.getAttribute("TCName"));
...
$(function(){
   ...
   $("#xiangguan").append("<li><a href='/Search2.html?queryString="+item+"'>"+item+"</a></li>");     
		...
		<script type="text/javascript">
		   ...
		   function handlerNextPage()
		   {
		      ...
		      location.href="/SeniorSearchAction2.html?queryString="+queryString+"&pageCount="+pageCount+"&currentPage="+pageCount+"&searchType="+searchType+"&TCName="+TCName+"&userid"+userid;
		   }
  ...
  <div class="con" style="margin-left:85px;" >
      ...
      <div class="searchbox">
       <form action="/SeniorSearchAction.html" method="post" onsubmit="return checkSubmit();"><input align="middle" name="queryString" class="q" id="queryString" value="<%=queryString %>" maxlength="100" size="50" autocomplete="off" baiduSug="1" /><input id="userid" name="userid" value="<%=userid %>" type="hidden"/><input id="searchType" name="searchType" value="<%=searchType %>" type="hidden"/><input id="TCName" name="TCName" value="<%=TCName %>" type="hidden"/><input name="re" type="hidden" value="1" /><input id="btn" style="background:#39a6df" class="btn" align="middle" value="搜索一下" type="submit" />
              </form>
      </div>
...
<div class="cl"></div>
   ...
   <div id="sopage">
		...
		<center>
			...
			if(i==1){
			%>
			<a class="this" style="color:#000" href="/SeniorSearchAction2.html?searchType=<%=searchType %>&TCName=<%=TCName %>&userid=<%=userid %>&queryString=<%=queryString %>&currentPage=<%=currentPage %>&pageCount=<%=currentPage %>"><%=currentPage %></a>
			<input id="nowpage" value=<%=currentPage %> hidden>
			<%
			} else{
			%>
			
			<a class="this" href="/SeniorSearchAction2.html?searchType=<%=searchType %>&TCName=<%=TCName %>&userid=<%=userid %>&queryString=<%=queryString %>&currentPage=<%=currentPage %>&pageCount=<%=currentPage %>"><%=currentPage %></a>
