 ...
 * @author fish 
 */  
public class GetZYSCAction extends ActionSupport  
{  
	...
	public void setNodeid(String nodeid) {
		...
		//¸ù¾Ýid
	@Action(value="getzysc2",results={@Result(name="success",type="json")})
	...
	{  
		...
		List<Collect> mDetails=this.baseservice.find(Collect.class, "Collect",keys,values);
		...
		for (int i = 0; i < mDetails.size(); i++) {
			...
			Collect collect=mDetails.get(i);
			
			map.put("zlmc", collect.getZymc());
			...
			map.put("zlms", collect.getZyms());
			map.put("zlid", collect.getZlid());
			map.put("filepath", collect.getPath());
			map.put("time", collect.getScsj());
